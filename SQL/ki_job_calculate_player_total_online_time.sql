...
DROP TEMPORARY TABLE IF EXISTS ki.temp_connect_pairs;
CREATE TEMPORARY TABLE ki.temp_connect_pairs 
...
DROP TEMPORARY TABLE IF EXISTS ki.temp_connect_pairs_clone;
CREATE TEMPORARY TABLE ki.temp_connect_pairs_clone LIKE ki.temp_connect_pairs;


INSERT INTO ki.temp_connect_pairs
SELECT
...
from ki.raw_connection_log t1 left join ki.rpt_overall_stats t2
...
INSERT INTO ki.rpt_overall_server_traffic (server_id, ucid)
select distinct t1.server_id, t1.ucid
from ki.temp_connect_pairs t1 left join ki.rpt_overall_server_traffic t2
...
UPDATE ki.rpt_overall_server_traffic s
LEFT JOIN 
...
INSERT INTO ki.backup_connection_log (id, server_id, session_id, type, player_ucid, player_name, player_id, ip_address, game_time, real_time)
...
FROM ki.raw_connection_log rcl
INNER JOIN ki.temp_connect_pairs tcp
	ON rcl.id = tcp.ci_id;
...
DELETE rcl.* FROM ki.raw_connection_log rcl
INNER JOIN ki.temp_connect_pairs tcp

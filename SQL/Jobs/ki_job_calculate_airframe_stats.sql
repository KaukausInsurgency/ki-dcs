...
DROP TEMPORARY TABLE IF EXISTS ki.temp_flight_time;
CREATE TEMPORARY TABLE ki.temp_flight_time 
(
	ucid varchar(128), 
    server_id int, 
    session_id int,
...
DROP TEMPORARY TABLE IF EXISTS ki.temp_flight_time_clone;
CREATE TEMPORARY TABLE ki.temp_flight_time_clone LIKE ki.temp_flight_time;

INSERT INTO ki.temp_flight_time
SELECT
  l1.ucid AS ucid,
...
INSERT INTO ki.temp_flight_time_clone
SELECT * FROM ki.temp_flight_time;
...
FROM ki.temp_flight_time t1 LEFT JOIN ki.rpt_airframe_stats t2
...
UPDATE ki.rpt_airframe_stats rpt 
INNER JOIN 
(
	SELECT ucid, role, SUM(co_time - ci_time) AS total_time
    FROM ki.temp_flight_time
	GROUP BY ucid, role
...
UPDATE ki.rpt_airframe_stats s
LEFT JOIN 
